...
var readline = require('readline'),
    ...
    internalCommands = {
        ...
        'stressCommit': {
            ...
            description: '\tExecutes the recorded batch as many times as requested, with delay (ms) between commands.',
            handler: function() {}
        }
    },
    rl;
...
function stressCommit(command, commands) {
    ...
    function executeBatch(index, callback) {
        async.series(executionBatch, callback);
    }

    executionBatch = stressBatch.reduce(addDelays, []);

    async.times(times, executeBatch, function() {
