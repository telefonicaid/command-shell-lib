...
function log() {
    if (!silent) {
        writer.log.apply(writer.log, arguments);
    }
}
 ...
 */
function stressInit() {
    log('Stress batch recording\n');
    stressTest = true;
    stressBatch = [];
}

/**
 * Checks that a command is valid and executes it based on the commands array given. An optional callback is passed
 * to the command.
 *
 ...
 */
function execute(command, commands, callback) {
    if (command.length - 1 !== commands[command[0]].parameters.length) {
        log('Wrong number of parameters. Expected: %s', JSON.stringify(commands[command[0]].parameters));
    } else {
        commands[command[0]].handler(command.slice(1), callback);
    }
}
...
function stressCommit(command, commands) {
    ...
    function executeCommandFn(value) {
        return function(callback) {
            log('Exec: ', JSON.stringify(value));
            if (commands[value[0]].asynchronous) {
                execute(value, commands, callback);
            } else {
...
function stressAdd(command) {
    log('Command added to stress batch\n');
...
function executeCommander(command, commands) {
    if (command[0] === 'help') {
        ...
        execute(command, commands, function() {});
    } else if (command[0] === '') {
        log('\n');
    } else {
...
function notImplemented() {
    log('This feature has not been fully implemented yet.');
    rl.prompt();
}
...
internalCommands = {
    'silent': {
        parameters: ['on'],
        description: '\tRemove the console output or reenables it again.',
        handler: changeSilent
    },
    'stressInit': {
        parameters: [],
